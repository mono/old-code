	...
	public class Tokenizer : yyParser.yyInput
	{
		...
		int adjust_real (int t)
		{
			...
			case Token.LITERAL_DECIMAL:
				try {
					val.val = System.Decimal.Parse (s, styles, csharp_format_info);
				} catch (OverflowException) {
					val.val = 0m;     
			...
			case Token.LITERAL_FLOAT:
				try {
					val.val = (float) System.Double.Parse (s, styles, csharp_format_info);
				} catch (OverflowException) {
					val.val = 0.0f;     
			...
			case Token.NONE:
				t = Token.LITERAL_DOUBLE;
				try {
					val.val = System.Double.Parse (s, styles, csharp_format_info);
				} catch (OverflowException) {
					val.val = 0.0;     
			...
			while ((c = getChar ()) != -1){
				if (c == '"'){
					if (quoted && peekChar () == '"'){
						...
						val.val = string_builder.ToString ();
		...
		private int consume_identifier (int s, bool quoted) 
		{
			...
			if (s >= 'a') {
				...
				val.val = ids;
				//Console.WriteLine ("{2} s: {0} e: {1}", val.sRow, val.eRow, ids);
				return keyword;
			}
			val.val = ids;
			return Token.IDENTIFIER;
